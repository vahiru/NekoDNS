...
export async function verifyTurnstile(env: Env, token?: string, ip?: string): Promise<TurnstileCheckResult> {
  if (!token) {
    console.error("Turnstile token missing", { ipPresent: Boolean(ip) });
  ...
  try {
    ...
    if (!result) {
      console.error("Turnstile siteverify returned invalid JSON", { status: response.status });
      return { success: false, errorCodes: ["invalid-siteverify-response"] };
    }
    if (result.success === true) return { success: true, errorCodes: [] };

    const codes = result["error-codes"] ?? [];
    console.error("Turnstile verification failed", { status: response.status, codes });
